  ...
  layers.fortress = {
    ...
    if (countries.includes("India")) {
        ...
        if (growthStocks.length > 0) {
            const weightPerStock = template.growth / growthStocks.length;
            growthVehicles = growthStocks.map((s) => ({
                ticker: s.symbol,
                weight: weightPerStock,
                why: `NSE ${s.sector}. MB Score: ${s.mbScore}/100. Tier: ${s.mbTier}.`,
            }));
        } else {
            // FALLBACK: Broad India Market if no specific scan results
            growthVehicles = [
                {
                    ticker: "NIFTYBEES",
                    weight: template.growth,
                    why: "Broad India market exposure. Best institutional fallback for NSE growth.",
                }
            ];
        }
    ...
    if (rocketStocks.length > 0) {
        const rocketWeight = template.upside / rocketStocks.length;
        upsideVehicles = rocketStocks.map((s) => ({
            ticker: s.symbol,
            weight: rocketWeight,
            why: `${s.sector}. Rocket-tier (highest growth potential, highest risk).`,
        }));
    } else {
        // FALLBACK: High-Beta Tech if no rocket stocks found
        upsideVehicles = [
            {
                ticker: "SMH",
                weight: template.upside,
                why: "Semiconductors ETF. High-growth proxy for technological moonshots.",
            }
        ];
    }
