...
**Momentum Radar** (`/momentum-radar`) — Dual-timeframe (Daily+Weekly) MACD(12,26,9) crossover scan across Nifty 500, sourced from the standalone `Equity_The-Final-chapter` bot. Read-only (symbol, CMP, EMA targets, stop loss) — no order execution exposed to web users. Bot's Zerodha auto-execution stays private, never reaches other users. Win-rate track record widget (Aug 4) surfaces `macd_signal_log` (open/hit_t1/hit_t2/stopped, already collected by the existing EOD resolver cron) via `GET /api/analysis/momentum-track-record` — stays hidden until at least one signal has resolved, since the strategy only went live recently. Fundamentals badge (Aug 4) shows PE / debt-to-equity / revenue growth under each signal's symbol — fetched from `yahoo-finance2` per unique symbol, 1hr in-memory cache, fails soft (badge just doesn't render if Yahoo is down or a field is missing).
