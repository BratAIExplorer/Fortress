...
**Latest:** **Session 30c (continued, July 30)** — ✅ **Bot Config Admin Panel Path Fix.** Fixed critical bug in `/momentum-radar/admin` credentials form: endpoint was writing Telegram/Zerodha keys to OLD location `/opt/macd-bot/.env` (stale) instead of CORRECT location `/opt/fortress/scripts/macd-bot/.env.local` (where bot reads from). **Root cause:** VPS was serving old compiled code. **Fix:** Empty commit to trigger GitHub Actions rebuild + redeploy. **Result:** Admin panel now correctly persists credentials to `/opt/fortress/scripts/macd-bot/.env.local`. Verified end-to-end: test credentials written → bot restarted with new creds → real Telegram credentials migrated from old location. Build: ✓ 0 errors. All 3 PM2 processes stable. **Session 30 (July 28)** — ✅ **Momentum Radar Tab Shipped.** Converted the standalone `Equity_The-Final-chapter` MACD crossover bot (`C:\The_Equity_Chapter\Equity_The-Final-chapter`) into a read-only Fortress tab. Decision: trading execution stays 100% on the owner's personal Zerodha keys in the standalone bot — the web tab only mirrors signals, no order placement exposed to other users (avoids per-user broker credential/compliance scope entirely). Gated sign-in only (nav link hidden when logged out, page + API re-check session server-side), 30-day free trial measured from `authUser.createdAt` (no new trial-start column needed), then requires `subscriptionStatus = 'active'` or shows a "Request Access" card. New additive-only DB migration (`drizzle/migrations/add-momentum-radar.sql`): `auth_user.subscription_status` (defaulted, existing 8 accounts unaffected) + new `macd_signals` table. Bot pushes its "Currently Active" snapshot to `POST /api/analysis/momentum-signals` over plain HTTP with a shared `x-cron-secret` (reused existing pattern, no new bot dependency — just `requests`, already imported) after each 5-min scan cycle; push is wrapped in try/except so a Fortress outage never breaks the bot's own trading loop. Build: ✓ 0 errors, `/momentum-radar` + `/api/analysis/momentum-signals` compiled clean. **All of the above deployed and validated same day (Session 30b):** ran `add-momentum-radar.sql` on prod Postgres (row-count checked before/after: 11→11, zero data loss), pushed + deployed to VPS, nav promoted to a top-level tab ahead of Fortress 30 per follow-up request, `.env` created for the bot with real `FORTRESS_API_URL`/`FORTRESS_CRON_SECRET`. **Also found and fixed two pre-existing infra bugs while validating:** (1) **CI/CD never actually rebuilt on deploy** — `.github/workflows/deploy.yml` built on the GitHub runner but never shipped that artifact anywhere; the VPS deploy step only did `git reset --hard` + `pm2 restart` against whatever `.next` was last built by hand. Every push since that workflow was written was silently serving stale code until a human SSH'd in and ran `npm run build` manually. Fixed by adding `npm ci && npm run build` into the VPS-side deploy script itself; validated by pushing a real commit and confirming `.next/BUILD_ID` changed automatically with no manual intervention. (2) **Duplicate, conflicting `CRON_SECRET` lines** in the VPS's `.env.production` (two different values). Confirmed via `pm2 env` which one was actually active, backed up the file, removed the stale line. **Note:** commit `d8209c46` (same day, separate session) subsequently opened Momentum Radar to all users and dropped the sign-in/trial gate described above — the gating code is still in git history if it needs to come back once auth/SMTP (Phase 3 backlog) is sorted. **Session 30c (same day):** deployed the bot itself to the VPS at `/opt/macd-bot` under PM2 (code copied via SSH, not git — the bot's GitHub repo belongs to a different account with no push access; Python 3.12 + deps installed). Added a password-gated `/momentum-radar/admin` page (`ADMIN_PANEL_PASSWORD` env var, not tied to user auth) showing bot health, plus a credentials form (`POST /api/admin/bot-config`) that writes Telegram/Zerodha keys directly into `/opt/macd-bot/.env` on the same VPS filesystem and restarts the bot — no SSH needed for future key changes or rotation. Validated the whole path with dummy Telegram credentials live in production: form → file write → PM2 auto-start → bot booted, correctly disabled order placement on incomplete Zerodha creds, logged a graceful (non-crashing) Telegram failure, zero restart-loop. Test process deleted and dummy values wiped afterward. **Real credentials still needed from the owner** — bot is running but idle until Telegram Bot Token/Admin ID and the three Zerodha fields are entered via the admin page; the Zerodha side additionally needs the VPS IP (`76.13.179.32`) whitelisted in the Kite Developer Portal before live order execution will work, and a one-time interactive Zerodha login (via Telegram) is required on first real run. **Also noted:** one deploy this session finished its VPS build but never reached the `pm2 restart` step (no visible error, GitHub Actions logs weren't reachable from here) — worked around manually; the CI/CD workflow could use an explicit "restart confirmed" check that fails loudly instead of trailing off silently. **Session 26 (July 21)** — ✅ **CI/CD Workflow Simplified + Package Lock Fixed.** (1) Simplified GitHub Actions validation (doesn't match other Fortress projects). (2) Fixed `package-lock.json` out-of-sync issue (yahoo-finance2 3.15.4→4.0.0, fetch-mock-cache 2.3.1→3.1.0, tough-cookie 5.1.2→6.0.2, filenamify 4.3.0→7.0.2, normalize-url 4.5.1→7.2.0). (3) VPS uses its own `.env.production` (already secure, not exposed). Workflow now: checkout → build → deploy (simpler, faster). Build: ✓ 9.8s, 0 errors. Phase 2 task created for optional GitHub Secrets hardening. Commits: 0f504692 (lock fix), 9d86ba4f (docs), e4ce2ebf (simplified), bdaa21b4 (initial fix). **Session 25 (July 21)** — ✅ **Hidden Gem Finder Data Handling Complete.** Diagnosed and fixed 4 issues with NSE/LSE ticker support: (1) Upgraded yahoo-finance2 v3.15.4 → v4.0.0 (strict validation fix for US tickers), (2) Added graceful degradation for NSE/LSE data unavailable instead of crash, (3) Implemented defensive filtering for partial data, (4) User-friendly error messages. **Result:** AAPL and US stocks return full real analysis; HDFC/CSPX/VUAA return honest "data unavailable" message instead of errors. Root cause identified: Yahoo Finance has poor NSE/LSE support (data quality/availability issue, not code bug). Scanner works for bulk because it filters bad rows; gem-score now does same. Build: ✓ 9.5s, 0 errors. Commits: c9725821 (v4.0.0), e68d266a (filtering), aa48cbe (graceful degradation). **Session 24 (July 21)** — ✅ **Auth Flows Complete.** Fixed critical gaps: logout endpoint, forgot-password with email, validation consolidation, rate limiting on password reset. All 6 end-to-end flows working. Commits: 75693184, 291b2277, 7c48b5a9. **Session 23 (July 21)** — Added LSE support (CSPX, VUAA, VWRA). Symbol resolver chain: US → NSE (.NS) → LSE (.L). Commits: cc73cf0b, c76db543. **Session 21 (July 20)** — Replaced Massive API with yahoo-finance2. Both markets now real data (NSE 480+, US 501+ rated). Fortress 30 live with real scores. Commit: 8e3e1410.  
...
## 📊 CURRENT STATE (July 21, 2026 — Session 24 Auth Flows Complete)
