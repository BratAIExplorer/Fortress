...
export async function GET(req: NextRequest) {
    try {
        ...
        // NEW: Enrich with TSYS sector conviction boost
        const enrichedResults = await Promise.all(
            rows.map(async (r) => {
                const boost = await getStockConvictionBoost(r.symbol, r.category);
                const boostedScore = Math.round(Number(r.mbScore) * (1 + boost.boost));
                ...
                return {
                    ...
                    l6: r.l6Pass,
                    mb_score: r.mbScore,
                    mb_score_boosted: boostedScore,
                    conviction_boost_pct: Math.round(boost.boost * 100),
                    thesis_context: boost.reason,
                    mb_tier: r.mbTier,
        ...
        return NextResponse.json({
            ...
            limit,
            meta: {
                thesis_enhanced: enrichedResults.filter((r) => r.conviction_boost_pct !== 0).length,
                total_stocks: enrichedResults.length,
            },
        });
    } catch (error) {
