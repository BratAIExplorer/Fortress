...
export async function getRandomWisdom(): Promise<Concept | null> {
    try {
        ...
        console.error("Error in getRandomWisdom:", error);
    }

    // Hardcoded fallback for "world-class customer experience" when DB is down
    return {
        id: "fallback-1",
        term: "The Fortress Mindset",
        definition: "Successful investing is not about being 'right' once; it's about not being 'wrong' fatally. Focus on protection first, and the growth will follow.",
        category: "Philosophy",
        source: "Fortress Intelligence"
    };
}
...
export async function getV5LowStocks(market: string = "NSE"): Promise<V5Stock[]> {
    ...
    if (market === "NSE") return v5LowStocks as V5Stock[];
    if (market === "US") return usV5LowStocks as V5Stock[];
    return [];
}
...
export async function getV5SubTenStocks(market: string = "NSE") {
    try {
        ...
        if (dbStocks.length === 0) {
            if (market === "NSE") return v5SubTenStocks;
            if (market === "US") return usV5PennyStocks.filter(s => (s.current_price || 0) < 20) as V5Stock[];
            return [];
        }
        return dbStocks;
    } catch (e) {
...
export async function getV5PennyStocks(market: string = "NSE") {
    try {
        ...
        if (dbStocks.length === 0) {
            if (market === "NSE") return v5PennyStocks;
            if (market === "US") return usV5PennyStocks;
            return [];
        }
        return dbStocks;
    } catch (e) {
...
export async function getTheses(market: string = "NSE"): Promise<ThesisRow[]> {
    const results = await db
        .select({
            id: schema.theses.id,
