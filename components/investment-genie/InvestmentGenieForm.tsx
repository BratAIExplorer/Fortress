...
import { STRATEGY_PRESETS } from "../../lib/investment-genie/presets";
...
export function InvestmentGenieForm({ onSubmit }: { onSubmit?: (profile: UserProfile) => void }) {
  ...
  const applyPreset = (presetId: string) => {
    ...
    if (profile.horizon) setHorizon(profile.horizon);
    if (profile.riskAppetite !== undefined) setRiskAppetite(profile.riskAppetite);
    if (profile.experience) setExperience(profile.experience);
    if (profile.incomeStability) setIncomeStability(profile.incomeStability);
    if (profile.countries) setCountries(profile.countries);
    if (profile.vehicles) setVehicles(profile.vehicles);
  };

  const submitPreset = (presetId: string) => {
    const preset = STRATEGY_PRESETS.find(p => p.id === presetId);
    if (!preset) return;
    ...
    const profile: UserProfile = {
      age,
      amount,
      horizon: presetProfile.horizon ?? horizon,
      experience: presetProfile.experience ?? experience,
      countries: presetProfile.countries ?? countries,
  ...
  return (
    <form aria-label="Investment Genie Form" onSubmit={handleSubmit} className="space-y-8">
      ...
      <div>
        <h3 className="text-sm font-medium text-white/70 mb-3">Quick Start: Strategy Templates</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {STRATEGY_PRESETS.map(preset => (
            <button
              key={preset.id}
              type="button"
              onClick={() => submitPreset(preset.id)}
              className={cn(
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <span className="text-xs text-white/40">PRESET</span>
              </div>
